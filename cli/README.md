...
Header-only command-line interface module using auto-registration pattern.

## Architecture

- **Header-only**: All commands are in `include/commands/`
- **Auto-registration**: Commands register themselves using inline variables
- **Registry pattern**: `CommandRegistry` manages command lifecycle
- **CLI11**: Used for argument parsing

## Usage as Library

Link against the `cli` target:

```cmake
target_link_libraries(your_app PRIVATE cli)
```
...
1. Create `include/commands/your_command.hpp`:
...
public:
    std::string name() const override { return "yourcommand"; }
    std::string description() const override { return "Description"; }
    
    void configure(CLI::App &sub) override {
        sub.add_option("-o,--option", option_, "Option description");
    }
...
// Auto-register
inline const bool registered_yourcommand =
    localpm::cli::CommandRegistry::instance()
        .register_type<localpm::cli::YourCommand>();
```

2. Add to `include/commands_all.hpp`:

```cpp
#include "commands/your_command.hpp"
```
...
For testing the CLI module independently:

```bash
mkdir build && cd build
cmake -DCLI_BUILD_STANDALONE=ON ..
cmake --build .
./bin/localpm-cli --help
```

## Existing Commands

- **init**: Initialize local package repository
- **add**: Add package to registry
- **list**: List packages
